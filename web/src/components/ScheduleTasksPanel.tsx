...
import { createScheduleTask, deleteScheduleTask, getScheduleTasks } from '@/api';
...
export default function ScheduleTasksPanel({ agents }: { agents: AgentInfo[] }) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 0.8fr 0.8fr 0.8fr 1fr auto', gap: 12, alignItems: 'end' }}>
          ...
          <Field label="目标 Agent">
            <select value={form.target_ip} onChange={(e) => setForm(prev => ({ ...prev, target_ip: e.target.value }))} style={selectStyle}>
              <option value="" style={{ background: '#151515', color: 'rgba(255,255,255,0.5)' }}>选择在线 Agent</option>
              {agents.map(agent => (
                <option key={agent.id || agent.ip_addr} value={agent.ip_addr} disabled={!agent.online} style={{ background: '#151515', color: 'rgba(255,255,255,0.58)' }}>
