...
在 macOS 上用 Docker Desktop 可以构建和打开 Web/API，但 `drop-agent` 看到的是 Docker Desktop 的 Linux VM，不是 macOS 宿主机进程；macOS 也没有 Linux perf/eBPF 接口。因此在 macOS 上跑真实采集任务失败是预期现象，建议只做前端/后端页面检查，真实 CPU/eBPF 演示放到 Ubuntu/Linux 机器。
...
前端“归因分析”页按证据、结论、可验证假设、追加采集四块展示；实际报告由采集任务完成后触发分析生成。
